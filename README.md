<!-- Premium Hero Banner -->
<p align="center">
  <!-- Make sure dark.svg and light.svg are committed in your repo root -->
  <!-- <img src="dark.svg" alt="Hero Banner Dark" width="100%" />
<img src="light.svg" alt="Hero Banner Light" width="100%" /> -->
</p>

<h1 align="center"> 
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35px"> 
  Hey there! I'm Archana Nair  
</h1>

<h3 align="center">🎓 BTech CSE | AI & ML Specialization | OIST Bhopal (Class of 2027)</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=F78A4C&center=true&width=500&lines=AI+Engineer...;Full+Stack+Developer...;GSSoC+Contributor+2026...;GSSoC+Legend...;Tech+Enthusiast..." />
</p>

---

### 🚀 About Me 

- 💻 **Passionate about:** AI Engineering, Software Development, Cybersecurity & UI/UX Design  
- 🌱 **Currently learning:** LLMs, RAG pipelines, MERN Stack  
- 🎨 **UI/UX Skills:** Figma & Canva  
- 🔐 **Cybersecurity Enthusiast:** Security risk management & proactive defense  
- ⚡ **Fun Fact:** I turn coffee ☕ into code!  

---

### 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,cpp,java,js,react,nodejs,mongodb,git,aws,figma" />
</p>

---

### 📚 Certifications  

- Generative AI & LLMs Virtual Internship (EduSkills)  
- Google/Coursera: Networks & Network Security  
- Deloitte Australia Technology Simulation (Forage)  
- She on Chain Bootcamp — Blockchain & Web3  
- Time Series Analysis — IIT Guwahati (Certificate of Merit)  
- LinkedIn Learning: *What Is Generative AI?*  

---

### ⚡ Projects  

- 🚀 **LLM-Powered RAG Application:** Context-aware document Q&A pipeline using LangChain, Hugging Face, and vector databases.  
- 🌐 **Full-Stack MERN Web Application:** JWT-authenticated, scalable MongoDB models, deployed end-to-end.  
- 🛡️ **CyberSecHub:** Exploring web security vulnerabilities.  
- 🎨 **Portfolio:** Showcasing my journey in UI/UX and development.  

---

### 🏆 Achievements  

- 🌟 **GSSoC Contributor 2026 & Legend** — Implemented features & CI/CD fixes in open-source RAG project.  
- 🏆 **Hackathons:** SIH 2025 project presented at CODE-2K26 (OIST).  
- 🎤 **Knowledge Sharing:** Led peer AI/ML sessions in university tech community.  

---

### 📊 Archana Nair's GitHub Stats  

<div align="center">
  <!-- Stats -->
  <!-- <img src="https://github-readme-stats.vercel.app/api?username=Archana7224&show_icons=true&theme=radical" width="400" alt="Archana's GitHub Stats"/> -->
  <br><br>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Archana7224&theme=radical" width="400" alt="Archana's GitHub Streak"/>
  <br><br>
  <!-- Contributions Snake -->
  <!-- <img src="https://raw.githubusercontent.com/Archana7224/Archana7224/output/snake.svg" alt="Snake animation" />
</div> -->

---

### 🌐 Let's Connect!  

<p align="center">
  <a href="https://www.linkedin.com/in/archana-nair-399bb0285/">
    <img src="https://img.shields.io/badge/LinkedIn-%230A66C2.svg?&style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/Archana7224">
    <img src="https://img.shields.io/badge/GitHub-171515.svg?&style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer&animation=twinkling&text=Thanks%20for%20visiting!&fontSize=24" width="100%" alt="Footer wave"/>
</p>
<p align="center"><i>Driven to innovate, grow, and contribute to the tech world.</i></p>
